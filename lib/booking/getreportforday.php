...
class GetReportForDay extends Response
{
    const BIRTHDAY       = 1;
    const ANIMATOR       = 2;
    const PHOTOGRAF      = 3;
    ...
    private function getReport(int $questId, array $arActive, array $arDate = []): array
    {
        ...
        if (count($arDate)) {
            ...
            $times          = $sheduleManager->getTimesForQuest($questId, $arDate[0], Tools::isWeekend($arDate[0]), $startFromZero);
        }
        $arTimes = [];
        foreach ($times as $time) {
            $arTimes[] = ['id' => $time, 'name' => $time];
        }
        reset($times);
        $this->response->addParam('times', $arTimes);
        ...
        foreach ($times as $time) {
            $tmpTime     = new DateTime($this->arDate[0] . ' ' . $time);
            $existsItems = $this->isExistItemForTime($arItems, $tmpTime);
            if (!$existsItems) {
                $tmpItems[] = $this->addEmptyItem($tmpTime);
            } else {
    ...
    private function isExistItemForTime(array $arItems, \Bitrix\Main\Type\Date $time)
    {
        $tmpTime  = $time;
        $tmpItems = [];
        ...
        foreach ($arItems as $item) {
            ...
            $dateTo = new DateTime($item['date'] . ' ' . $item['time']);
            $dateTo->add('1 hour');
            if ($tmpTime >= $date && $tmpTime < $dateTo) {
                $tmpItems[] = $item;
            }
        }
        if (count($tmpItems) > 0) {
     ...
     * Пустой массив
     *
     * @param \Bitrix\Main\Type\Date $dateTime
     * @return array
     * @throws \Bitrix\Main\ArgumentException
     * @throws \Bitrix\Main\ObjectPropertyException
     * @throws \Bitrix\Main\SystemException
     */
    private function addEmptyItem(\Bitrix\Main\Type\Date $dateTime): array
    {
        $arServices = [];
        foreach ($this->arAddServices as $key => $arAddService) {
            $arServices[$key] = [
        ...
        $result = [
            ...
            'email'         => '',
            'clientName'    => '',
            'utm'           => $this->setSelected($this->arSourceValue, 0),
            'manager'       => $this->setSelected($this->arManager, 0),
            'status'        => $this->setSelected($this->arStatusValue, 0),
            'additional'    => $arServices,
            'saleReason'    => $this->setSelected($this->arSaleReason, 0),
            'comment'       => '',
            'reserved'      => false,
            'maxPlayers'    => $this->getMaxPlayers(),
        ];
    ...
    private function execCalculator(array $booking, array $arServices): array
    {
        ...
        if (strlen($booking['CCERT_NUM']) > 2) {
            ...
            if ($arPrices['CERT_VALUE'] == self::FULLPRICE_CODE) {
